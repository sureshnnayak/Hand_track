...
def check_hand(results,images,dev):
    if results.multi_hand_landmarks:
        ...
        for handLms in results.multi_hand_landmarks:
            ...
            if hand_side =="Left" and topLeft != -1 :
                ...
                return x_meter,y_meter, mfk_distance
                pub.publish(ros_msg)
            #======================================ROS====================================
        """
        if dev:
...
while not rospy.is_shutdown():
    ...
    dev = 1
    x_meter, y_meter, z_meter = check_hand(results,images,dev)
    display_image(images)

    hand_info= "'hand':({0},{1},{2}),".format(x_meter, y_meter, z_meter)
    key = cv2.waitKey(1)
